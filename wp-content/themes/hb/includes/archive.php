...
if ( isset($query) && isset($post_type) ) :
	...
	while ( $query->have_posts() ) {
		...
		if ( has_post_thumbnail() ) {
			...
			$list_accumulator .= '<div class="col-xs-10 col-sm-3 image">'.get_the_post_thumbnail( ).'</div></div>';
		}
		
		$list_accumulator .= '<div class="col-xs-10 col-sm-7 col-md-7 col-lg-5 text">'
						  .  '<h3>'.get_the_title().'</h3>';
